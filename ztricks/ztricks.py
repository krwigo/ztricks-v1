...
def load():
	...
	# we need to track names too!
	es.sql('query','ztricks',"ALTER TABLE points ADD COLUMN playername VARCHAR(200) NOT NULL DEFAULT 'unknown player'")
...
def givePlayerPoints(userid, points):
	...
	es.sql('query','ztricks',"UPDATE points SET points=(points + %s), playername='%s' WHERE playersteam='%s'" % (points, safePlayerName(userid), steamid))
