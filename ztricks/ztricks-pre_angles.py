import es, os, re, playerlib, gamethread, effectlib, urllib, time, popuplib, sqlite3
...
def foundTrigger(userid, triggerindex):
	...
	for trick in tricks:
		...
		t=compareList(pathlist, passlist, userid)
		if t < 1: continue
	...
	foundTrick(userid, best_index, best_time)
...
def foundTrick(userid, trickindex, time_first):
	#
	...
	trick_name=trickName(name, players[userid]['trick_count'])
	s=es.sql('queryvalue','ztricks',"SELECT seconds FROM records WHERE trickname='%s'" % trick_name)

	es.msg("#multi", "#lightgreen%s just completed #green%s#lightgreen !! it took #green%.4f#lightgreen seconds !! #green%.4f#lightgreen difference !!" % (getPlayerName(userid), trick_name, seconds, (float(seconds) - float(s))))
...
def compareList(pathlist, passlist, userid):
	...
	if len(pathlist) > len(userlist): return -1
	delta=int("-%s" % len(pathlist))

	# a userlist may not end in a passlist item
	if userlist[-1] in passlist: return -2
		...
		for box in tricks:
			...
			if name == req:
				...
				for trigger in namelist:
					...
					[id, name, coord1, coord2]=getTriggerArray(trigger)
					c=centerof(coord1, coord2)

					[id, name, coord1, coord2]=getTriggerArray(lasttrigger)
