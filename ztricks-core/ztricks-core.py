...
def foundTrigger(userid, trigger_shape, trigger_name, trigger_id, x, y, z, player_velocity):
	...
	for name in tricks():
		...
		if cfg('tricks', name, 'iId') == 0:
			disableTrick(name, 'No id defined')
			continue
...
def writeconfig():
	...
	#vlog("last map = [%s]" % last_map)
	#if last_map == None or last_map == "None":
	#	msg("No config, skipping?")
	#	return
	#
	# /cstrike/addons/whatever/cfg/mapname.cfg
	#if not os.path.isdir("%s/cfg" % our_dir):
	#	os.mkdir("%s/cfg" % our_dir)
	#configfile = "%s/cfg/%s.cfg" % (our_dir, last_map)
...
def map_changed():
	...
	# load this maps config
	loadConfig()

def getMap():
	return "%s" % es.ServerVar("eventscripts_currentmap")
...
def timer3():
	...
	#vlog("last_map = [%s] %s" % (last_map, type(last_map)))
	#vlog("this map = [%s] %s" % (getMap(), type(getMap())))
	if not last_map == getMap():
		last_map = getMap()
		map_changed()

	gamethread.delayedname(1, 'timer3', timer3)
...
def compareList(pathlist, passlist, userid):
	...
	for point in userlist:
		# if this is the same as last, skip it
		# fixes 2,21,2,21 issues hopefully :/
		if (point == last_point): continue
		...
		if not point in passlist:
			...
			# only update last point here because
...
def playerReset(userid, keep_last=False):
	...
	if len(players[userid]['triggerlist']) >= 50:
		for n in ['triggerlist', 'triggertimes', 'triggerangles', 'triggerspeeds']:
			players[userid][n] = players[userid][n][-30:]

	
	# 2009 04 23 - instead of removing trig and tricks, just remove combo?
	#if keep_last:
...
def zadminmenuselect(userid, choice, popupid):
	...
	try:
		...
		if args[0] == 'help':
			...
			myPopup.addline('   ')
			myPopup.addline('The menu options for Set Sphere Height, Set Sphere Radius,')
			myPopup.addline('and Set Coords use your position to calculate. So stand where')
			...
			if c == 1:
				...
				if args[2] == 'Change Enabled':
					...
					# Since the path has changed, lets disable the trick :/ its for safety
					if cfg('tricks', name, 'bEnabled'):
						disableTrick(name, 'Path removal')
