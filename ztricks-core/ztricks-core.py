...
def zsetid_trick():
	...
	if text == None:
		es.tell(userid, 'Usage: zsetid_trick "the name of the trick" 200')
...
def zadminmenuselect(userid, choice, popupid):
	...
	if args[0] == 'help':
		...
		if c == 1:
			...
			for op in ['View current values', 'Change Enabled', 'Draw Boxes', 'Add trigger to pass list', 'Remove trigger from pass list', 'View pass list', 'Add trigger to path list', 'Remove trigger from path list', 'View path list', 'Change point value']:
		...
		if c == 1:
			...
			for x in ['Change Shape', 'Set Coord1', 'Set Coord2', 'Set Radius', 'Change Combo End', 'Change Enabled', 'Change Symetrical', 'Set Max Velocity', 'Set Min Velocity', 'Draw It', 'Show Raw Data', 'Set Sphere Height']:
