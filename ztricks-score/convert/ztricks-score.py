import ConfigParser, time, sys, traceback, shelve, re
ver=2
#our_dir=es.getAddonPath('ztricks-score')
raw = ConfigParser.RawConfigParser()
raw.read("score.ini")

d = shelve.open("converted.shv")
...
for section in raw.sections():
	...
	for (key, value) in raw.items(section):
		...
		elif isFloat(value):	value = float(value)
		
		d[section][key] = value
		
		print "- item:%s value:%s type:%s" % (key, value, type(value))

#quit()

print "###"
print "dumping current dataset"
print "###"
...
def ztricks_trick(e):
	...
	if fastest_time == 0:
		...
		fastest_time = ptime
		setValue(name, 'fastest_time_first_time', ptime)
		...
		setValue(name, 'fastest_time_time', ptime)
		...
		slowest_time = ptime
		setValue(name, 'slowest_time_first_time', ptime)
		...
		setValue(name, 'slowest_time_time', ptime)
...
def setValue(section, key, value):
	...
	try:
		...
		raw.setValue(section, key, value)
