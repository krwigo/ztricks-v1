...
def ztricks_trigger(e):
	...
	set(steamid, 'name', gpn(userid))
	set(steamid, 'last_seen', time.time())
...
def ztricks_trick(e):
	...
	name = e['trick_name']
	ptime = float(e['trick_time'])
	speed = float(e['player_speed'])
	playername = gpn(userid)
	points = float(e['trick_points'])
	
	# Give them a point
	es.server.queuecmd("es score add %s 1" % e['userid'])
	
	# Give them points
	set(steamid, 'points', (getValue(steamid, 'points', 0) + points))
		
	# Say the message
	es.msg("#multi", "#lightgreen%s#default just completed #lightgreen%s#default going #green%.0f#default mph in #lightgreen%.4f#default seconds!!" % (playername, name, speed, ptime))

	# Total count
	# FIXME?
	#cnt = int(getValue(name, 'total_completed', 0))
	#set(name, 'total_completed', (cnt + 1))

	###########################################
	# First person to do the trick
	###########################################
	if getValue(name, 'first_player_time', 0) == 0:
		set(name, 'first_player_time', time.time())
		set(name, 'first_player_name', playername)
		set(name, 'first_player_steamid', steamid)
	...
	if fastest_speed == 0:
		...
		fastest_speed = speed
		set(name, 'fastest_speed_first_mph', speed)
		set(name, 'fastest_speed_first_name', playername)
		set(name, 'fastest_speed_first_steamid', steamid)
		set(name, 'fastest_speed_first_date', time.time())
		set(name, 'fastest_speed_mph', speed)
		set(name, 'fastest_speed_name', playername)
		set(name, 'fastest_speed_steamid', steamid)
		set(name, 'fastest_speed_date', time.time())
		slowest_speed = speed
		set(name, 'slowest_speed_first_mph', speed)
		set(name, 'slowest_speed_first_name', playername)
		set(name, 'slowest_speed_first_steamid', steamid)
		set(name, 'slowest_speed_first_date', time.time())
		set(name, 'slowest_speed_mph', speed)
		set(name, 'slowest_speed_name', playername)
		set(name, 'slowest_speed_steamid', steamid)
		set(name, 'slowest_speed_date', time.time())
	...
	if fastest_time == 0:
		...
		fastest_time = ptime
		set(name, 'fastest_time_first_time', ptime)
		set(name, 'fastest_time_first_name', playername)
		set(name, 'fastest_time_first_steamid', steamid)
		set(name, 'fastest_time_first_date', time.time())
		set(name, 'fastest_time_time', ptime)
		set(name, 'fastest_time_name', playername)
		set(name, 'fastest_time_steamid', steamid)
		set(name, 'fastest_time_date', time.time())
		slowest_time = ptime
		set(name, 'slowest_time_first_time', ptime)
		set(name, 'slowest_time_first_name', playername)
		set(name, 'slowest_time_first_steamid', steamid)
		set(name, 'slowest_time_first_date', time.time())
		set(name, 'slowest_time_time', ptime)
		set(name, 'slowest_time_name', playername)
		set(name, 'slowest_time_steamid', steamid)
		set(name, 'slowest_time_date', time.time())

	elif ptime < fastest_time:
		diff = fastest_time - ptime
	...
	elif ptime > slowest_time:
		diff = ptime - slowest_time
	...
	set(steamid, "count %s" % name, count)
